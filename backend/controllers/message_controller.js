...
exports.index = async (req, res, next) => {
  try {
    const threads = await Thread.find({users: req.user.user_id})
    .populate('users','username first_name last_name avatar')
    res.send(threads)
  } catch (e) {
    next(e)
  }
...
exports.show = async (req, res, next) => {
  try {
    const thread = await Thread.find({$and: [{users: req.user.user_id}, {users: req.params.id}]})
    ...
    if(thread){
      ...
      messages.reverse()
      res.json({thread, messages, count, cursor: Number(req.query.cursor ) + 10})
    } else {
      res.json(thread)
    }
  } catch(e) {
...
exports.create = async(req, res, next) => {
  try {
    let [thread] = await Thread.find({$and: [{users: req.user.user_id}, {users: req.body.to_id}]})
    .populate('users','_id username first_name last_name avatar')
    if(!thread){
     ...
     const [_created, message, notification, notifiee] = await Promise.all([
        ...
        await Message.find({thread_id: thread._id})
