...
import {usePostsQuery, useProfilePostsQuery} from '../../components/hooks/usePostsQuery';
...
const PostCards = ({currentUser, posts}) => {
  ...
  useEffect(() => {
    const handleScroll = (e) => {
      if(window.innerHeight + window.scrollY >= document.body.offsetHeight && 
        hasNextPage && location.pathname === '/') {
        fetchNextPage()
      }
      if(window.innerHeight + window.scrollY >= document.body.offsetHeight && 
        profile.hasNextPage && location.pathname === '/profile') {
  ...
  if(posts) return posts.length > 0 ?
  posts.map( post => 
  <div 
    key={post._id} 
