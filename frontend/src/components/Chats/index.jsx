...
import {
  ...
  setSearchResults,
  setFriend,
} from "../../pages/Messages/features/messagesSlice";
...
const Chats = () => {
  ...
  const handleSearch = (e) => {
    ...
    const [results] = friendQuery.data.pages.map((page) =>
      page.friends.filter((friend) => {
  ...
  const pickMessage = threadQuery.data?.map((thread) => {
    const friend = thread.users.filter(
    ...
    return (
      ...
      >
        ...
        <Typography ml={1}>{friend?.username}</Typography>
      </Button>
    );
  });
  ...
  return (
    <div className="chats-section">
      ...
      {searchResults.length > 0 && (
        <div className="scroll">{pickFromSearch}</div>
      )}

      {threadQuery.isLoading ? (
        <FriendPlaceholder />
      ) : (
        <div className="scroll flex-grow">{pickMessage}</div>
      )}
    </div>
  );
};
