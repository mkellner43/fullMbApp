      ...
      const newPages = old.pages.map(page => {
        return {
          ...
          notifications: page.notifications.map(notification => 
            notification._id === variables ?
            {...notification, status: 'read'} : notification)      
        }
      })
        return {
          ...old,
...
const handleRead = (id) => {
  const [alreadyRead] = queryClient.getQueryData(['notifications']).pages.map(page => page.notifications.find(notification => 
    notification._id === id && notification.status === 'unread'))
    if(alreadyRead) readOneNotification.mutate(id)
}

const handleScroll = () => {
 if(scrollPosition.current.scrollHeight === scrollPosition.current.scrollTop + scrollPosition.current.clientHeight && query.hasNextPage)
  query.fetchNextPage()
}

  return (
    <div className='notification-section' ref={scrollPosition} onScroll={handleScroll}>
      {  notificationQuery?.pages?.map((page, indx) => page.notifications.map((notification, idx) => 
        { 
        return <div 
      ...
      { (scrollPosition.current?.scrollHeight <= scrollPosition.current?.clientHeight ||  !scrollPosition.current ) &&query.hasNextPage && 
        <Button onClick={() => query.fetchNextPage()}>Load More</Button> }
      { query.isFetchingNextPage && <RefreshIcon />}
      { notificationQuery?.isSuccess && notificationQuery?.pages[0].length > 0 &&
