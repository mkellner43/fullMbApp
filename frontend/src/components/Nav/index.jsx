...
import { Badge, Tooltip, Snackbar, Alert, Box, Drawer, List,
  Typography, Divider, IconButton, ListItem, ListItemButton,
  ListItemIcon, ListItemText, useTheme } from '@mui/material';
import { Menu, ChevronLeft, ChevronRight, Logout,
  Feed, Message, PeopleAlt, Notifications } from '@mui/icons-material';
...
export const Nav = ({currentUser}) => {
  ...
  useEffect(() => {
    ...
    switcher === 'dark' ? dispatch(setTheme('dark')) : dispatch(setTheme('light'))
  }, [switcher, dispatch])
  ...
  return (
    <Box>
      ...
      <AppBar open={open} sx={{display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', minHeight: '60px', padding: '0rem 1rem'}}> 
        <IconButton
          color="inherit"
        ...
        <div style={{display: 'flex', alignItems: 'center'}}>
          <Tooltip title='Profile' placement='bottom-end' arrow>
            <Box sx={{display: 'flex', alignItems: 'center', justifyContent: 'space-evenly', width: 1, p: 2}}
             onClick={() => redirectToProfile({id: currentUser.id, user: currentUser})}
             >
            <AvatarWithStatus user={currentUser} width={'3rem'} height={'3rem'} />
