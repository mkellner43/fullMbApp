...
const MessageForm = () => {
  ...
  const sendMessageQuery = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      socket.emit('notification', {to_id: variables.to_id, type: 'Message', msg: `${currentUser.username} : ${variables.message}`})
    },
    ...
    onMutate: async(variables) => {
      ...
      queryClient.setQueryData(['thread:', variables.to_id], (old) => {
        const newPages = old.pages.map((page, idx) => idx === 0 ? 
        { ...page,
          messages: 
          [ ...page.messages, 
            { 
              ...
              sender: {...currentUser, _id: currentUser.id}, _id: 'newMessage'
            }
          ] 
        }
        : 
