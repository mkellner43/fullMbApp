...
import { updateUsername, updatePassword, setUsernameError, setPasswordError, setMainError, setCurrentUser, setToken } from './features/loginSlice';
...
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    !username ? dispatch(setUsernameError('Username is required')) : dispatch(setUsernameError(null))
    !password ? dispatch(setPasswordError('Password is required')) : dispatch(setPasswordError(null))
  ...
  return (
    <form className='center'>
      <div className="page-width-form center">
        <Typography variant='h2' component='h1' textAlign='center' sx={{m: 4}}>
          Log In
        </Typography>
      ...
      {userCreated && <Snackbar open={userCreated ? true : false} onClose={handleClose} anchorOrigin={{ vertical: 'top', horizontal: 'center'}}>
        ...
        <Button variant='outlined' type='button' onClick={() => navigate('/signup')} sx={{m: 1}}>
          Sign up
        </Button>
      </div>
    </form>
